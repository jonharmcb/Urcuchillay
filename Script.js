const questions = [
    ...
    {   question: "What is the only planet in our solar system that rotates clockwise?",
        options: ["Venus", "Mars", "Uranus", "Jupiter"],
        answer: "Venus",
        correctExplanation: "Correct: Venus is the only planet that rotates clockwise",
        incorrectExplanation: "Incorrect: Venus is the only planet that rotates clockwise"
    },
    {   
        question: "What is a group of cats called?",
        options: ["A Clowder", "A Murder", "A Prickle", "A Mischief", "An Embarrassment"],
        answer: ["A Clowder"],
        correctExplanation: "Correct, A Swarm of crows is a Murder, Hedgehogs = Prickle, Rats = A Mischief, Pandas = An Embarrassment",
        incorrectExplanation: "Incorrect, Cats = A Clowder, Crows = A Murder, Hedgehogs = A Prickle, Rats = A Mischief, Pandas - An Embarrassment",
    },
    {   
        question: "What is a fear of long words called?",
        options: ["Hippopotomonstrosesquippedaliophobia", "Arachnophobia", "Phobophobia", "Claustrophobia", "Globophobia"],
        answer: "Hippopotomonstrosesquippedaliophobia",
        correctExplanation: "Correct: Hippopotomonstrosesquippedaliophobia is ironically the fear of long words",
        incorrectExplanation: "Incorrect: Hippopotomonstrosesquippedaliophobia is ironically the fear of long words, Arachnophobia = Spiders, Phobophobia = Fear of phobias, Claustrophobia = Close spaces, Globophobia = Balloons"
    },
    
];


let currentQuestions = []; // Array to store the 10 random questions
...
document.addEventListener('DOMContentLoaded', function() {
    function selectRandomQuestions() {
        ...
        currentQuestions = [];
        for (let i = 0; i < 10 && tempQuestions.length > 0; i++) {
            const randomIndex = Math.floor(Math.random() * tempQuestions.length);
            currentQuestions.push(tempQuestions.splice(randomIndex, 1)[0]);
        }
    }
    ...
    function checkAnswer(selectedOption) {
        const q = currentQuestions[currentQuestionIndex]; // Use currentQuestions
        let explanation = "";
    ...
    function showDetailedResults() {
        ...
        resultElement.innerHTML = resultsHTML + `<p>Your score: ${score} out of ${currentQuestions.length}</p>`;
    }
